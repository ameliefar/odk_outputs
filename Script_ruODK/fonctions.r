...
deplacer_dossier <- function(dossier_source, dossier_cible) {
	...
	#		Création fonction
	dossier_source = normalizePath(
		file.path(path = dossier_source),
		mustWork = FALSE,
		winslash = "\\"
	)
	dossier_cible = normalizePath(
		file.path(path = dossier_cible),
		mustWork = FALSE,
		winslash = "\\"
	)
	...
	if (!(dir.exists(dossier_source)) | (dir.exists(dossier_cible))) {
		print(
			"La commande deplacer_dossier est en erreur - Dossier source inexistant ou dossier cible déjà présent"
		)
		return(FALSE)
	}
		...
		if (file.info(fic)$isdir) {
			...
			if (
				!deplacer_dossier(
					...
					dossier_cible = normalizePath(
						file.path(path = dossier_cible, basename(fic)),
						mustWork = FALSE,
						winslash = "\\"
					)
				)
...
formulaire_existe <- function(nom_du_projet, nom_du_formulaire) {
	...
	if (projet_existe(nom_du_projet)) {
		...
		if (
			nrow(subset(
				form_list(pid = pid_projet(nom_du_projet)),
...
initialise_service_recuperation <- function(nom_du_projet, nom_du_formulaire) {
	...
	if (formulaire_existe(nom_du_projet, nom_du_formulaire)) {
		if (last_submission_isna(nom_du_projet, nom_du_formulaire)) {
			...
			ru_setup(
				pid = pid_projet(nom_du_projet),
				fid = fid_formulaire(nom_du_projet, nom_du_formulaire),
				odkc_version = version_odk_central
			)
			if (
				doublon_nom_champ_formulaire(
					nom_du_projet = nom_du_projet,
					nom_du_formulaire = nom_du_formulaire
				)
			) {
				print(
					"Le formulaire comporte des champs en doublons et ne peut être traité avec ce code"
				)
				return(FALSE)
			} else {
				return(TRUE)
...
schema_formulaire <- function(mode_extraction, info_geom = NULL) {
	...
	df_schema = (subset(
		form_schema(),
		type != const_type_structure & name != const_instance_id
	))
	...
	for (i in 2:nrow(df_schema)) {
		...
		#cherche parent :
		for (j in (i - 1):1) {
			#Remonte le tableau du schema pour trouver le parent
			...
			if (
				(startsWith(ligne$path, paste0(df_schema[j, ]$path, "/"))) || (j == 1)
			) {
				...
				if (endsWith(parent_courant, "/")) {
					#Excepté sur la première ligne pour la racine la colonne path ne se termine pas par un "/"
				...
				nom_colonne_fichier_courant = gsub(
					"/",
					separateur_groupe,
					substring(ligne$path, nchar(parent_courant) + decalage)
				)
				nom_colonne_df_courant = gsub(
					"/",
					".",
					substring(ligne$path, nchar(parent_courant) + decalage)
				)
		...
		if (ligne$type == "repeat") {
			...
			if (mode_extraction == const_mode_zip) {
				...
				fichier_courant = paste0(
					const_submissions,
					".",
					gsub("/", ".", substring(ligne$path, 2)),
					".csv"
				)
			}
		...
		if (mode_extraction == const_mode_zip) {
			...
			for (k in 2:nrow(df_schema)) {
				...
				if (ligne$type == const_type_geopoint) {
					...
					for (suffixe in champ_type_geom_zip) {
						...
						ligne$nom_colonne_fichier = paste0(
							df_schema[k, ]$nom_colonne_fichier,
							"-",
							suffixe
						)
						ligne$nom_colonne_df = paste0(
							df_schema[k, ]$nom_colonne_df,
							".",
							suffixe
						)
					...
					#On reprend telle quel la ligne en cours
					df_tmp = rbind(df_tmp, ligne)
				}
			}
		} else {
			...
			for (k in 2:nrow(df_schema)) {
				...
				if (ligne$type == const_type_geopoint) {
					...
					if (
						subset(info_geom, nom_champ_geom == ligne$ruodk_name)$renseigne ==
							TRUE
					) {
						#On ajoute 4 lignes
	...
	tryCatch(
		{
			...
			df_data = read.csv(
				normalizePath(
					file.path(
						dossier_sortie,
						const_sous_dossier_tmp,
						df_fichier$nom_fichier
					),
					mustWork = FALSE,
					winslash = "\\"
				),
			...
			if (df_fichier$niveau == 0) {
				#Cas du fichier principal
				if (mode_extraction == const_mode_zip) {
					#Fichier principal en mode zip
					if (ligne_fin == 2) {
						colonne_a_extraire = c(df_obligatoire_zip$colonne_origine) #pas de champs en dehors de df_obligatoire
					} else {
						colonne_a_extraire = c(
							df_obligatoire_zip$colonne_origine,
							df_colonnes[2:ligne_fin, ]$nom_colonne_df
						) #il ne faut pas prendre la première ligne
						print(colonne_a_extraire)
					}
				} else {
					#fichier principal en mode API
					if (ligne_fin == 2) {
						colonne_a_extraire = c(df_obligatoire_api$colonne_origine) #pas de champs en dehors de df_obligatoire
					} else {
						colonne_a_extraire = c(
							df_obligatoire_api$colonne_origine,
							df_colonnes[2:ligne_fin, ]$ruodk_name
						) #il ne faut pas prendre la première ligne
					}
				}
			} else {
				#Fichier représentant une boucle de répétitions
				if (mode_extraction == const_mode_zip) {
					#Fichier boucle en mode zip
					colonne_a_extraire = c(
						df_obligatoire_zip_repeat$colonne_origine,
						df_colonnes[2:nrow(df_colonnes), ]$nom_colonne_df
					) #il ne faut pas prendre la première ligne
				} else {
					#fichier boucle en mode API
					...
					cle_etrangere = str_to_lower(paste0(
						gsub('.{4}$', '', gsub('[.]', "_", df_fichier$fichier_parent)),
						"_id"
					))
					df_obligatoire_api_repeat = data.frame(
						colonne_origine = c(cle_etrangere, "id"),
						colonne_sortie = c("uuid_parent", "uuid")
					)
					colonne_a_extraire = c(
						df_obligatoire_api_repeat$colonne_origine,
						df_colonnes[2:nrow(df_colonnes), ]$nom_colonne_fichier
					)
				}
			}

			if (mode_full) {
				#Analyse la nécessité de récupérer les autres colonnes systèmes que celles obligatoires déjà listées. Ces colonnes seront placées en fin de dataframe
				autres_colonnes_system = colnames(df_data)[
					!(colnames(df_data) %in% colonne_a_extraire)
				]
				df_data = subset(
					df_data,
					select = c(colonne_a_extraire, autres_colonnes_system)
				)
			} else {
				#Mode simplifié
			...
			if (mode_extraction == const_mode_zip) {
				if (df_fichier$niveau == 0) {
					...
					for (col in colnames(df_data)) {
						if (col %in% df_obligatoire_zip$colonne_origine) {
							#traitement des colonnes systèmes obligatoires
							if (exists("entete_sortie")) {
								#Utile pour la première colonne car entete_sortie n'existe pas à ce moment - En mode zip cette colonne est SubmissionDate
								entete_sortie = c(
									entete_sortie,
									subset(
										df_obligatoire_zip,
										colonne_origine == col,
										select = colonne_sortie
									)$colonne_sortie
								)
							} else {
								entete_sortie = c(
							...
							#traitement des colonnes représentant les champs de formulaire.
							if (col %in% df_colonnes$nom_colonne_df) {
								#Traitement des colonnes du formulaire
								entete_sortie = c(
									entete_sortie,
									subset(df_colonnes, nom_colonne_df == col, select = name)$name
								)
								#Traitement particulier pour les colonnes de type media
							} else {
								#traitement des autres colonnes s'il y a lieu
								entete_sortie = c(entete_sortie, col)
							}
						}
					}
				} else {
					#Niveau > 0 - Le fichier comporte déjà PARENT_KEY et KEY puis les colonnes formulaires . Le fichier représente une boucle de répétitions.
					for (col in colnames(df_data)) {
						if (col %in% df_obligatoire_zip_repeat$colonne_origine) {
							#traitement des colonnes systèmes obligatoires (PARENT_KEY et KEY
							if (exists("entete_sortie")) {
								#Utile pour la première colonne car entete_sortie n'existe pas à ce moment
								entete_sortie = c(
									entete_sortie,
									subset(
										df_obligatoire_zip_repeat,
										colonne_origine == col,
										select = colonne_sortie
									)$colonne_sortie
								)
				...
				if (df_fichier$niveau == 0) {
					#Fichier principal
					for (col in colnames(df_data)) {
						if (col %in% df_obligatoire_api$colonne_origine) {
							#traitement des colonnes systèmes obligatoires
							if (exists("entete_sortie")) {
								#Utile pour la première colonne car entete_sortie n'existe pas à ce moment -
								entete_sortie = c(
							...
							#traitement des colonnes représentant les champs de formulaire.
							if (col %in% df_colonnes$ruodk_name) {
								#Traitement des colonnes du formulaire
								entete_sortie = c(
									entete_sortie,
									subset(df_colonnes, ruodk_name == col, select = name)$name
								)
								#Traitement particulier pour les colonnes de type media
							} else {
								#traitement des autres colonnes s'il y a lieu
					...
					#Niveau > 0 - Le fichier comporte une colonne id et une autre pour la clé étrangère (lien au fichier parent) avec une syntaxe spécifique. Le fichier représente une boucle de répétitions.
					for (col in colnames(df_data)) {
						if (col %in% df_obligatoire_api_repeat$colonne_origine) {
							#traitement des colonnes systèmes obligatoires (PAREN?T_KEY et KEY
							if (exists("entete_sortie")) {
								#Utile pour la première colonne car entete_sortie n'existe pas à ce moment
								entete_sortie = c(
									entete_sortie,
									subset(
										df_obligatoire_api_repeat,
										colonne_origine == col,
										select = colonne_sortie
									)$colonne_sortie
								)
							...
							#traitement des colonnes représentant les champs de formulaire. !!! remplacer nom_colnne_df par nom_colonne_fichier ?????
							if (col %in% df_colonnes$nom_colonne_fichier) {
								#Traitement des colonnes du formulaire
								entete_sortie = c(
									entete_sortie,
									subset(
										df_colonnes,
										nom_colonne_fichier == col,
										select = name
									)$name
								)
								#Traitement particulier pour les colonnes de type media
			...
			write.table(
				df_data,
				normalizePath(
					file.path(dossier_sortie, df_fichier$nom_fichier),
					mustWork = FALSE,
					winslash = "\\"
				),
				row.names = FALSE,
	...
	tryCatch(
		{
			...
			erreur = FALSE
			position_colonne = 0
			for (col in nom_colonne_origine) {
				#Pour chaque colonne du fichier normalisé
				...
				if (col %in% df_colonnes$name) {
					...
					if (type_colonne == "binary") {
						#Traitement particulier pour les colonnes de type media => binary
						for (i in 1:nrow(df_data)) {
							#Vérifie l'existence des fichiers listés dans la colonne entière
							df_media_courant = df_data[i, col]
							if (!(is.na(df_media_courant))) {
								#La colonne est renseignée (un fichier a normalement été uploadé
								chemin_media = normalizePath(
									file.path(
										dossier_sortie,
										const_dossier_media,
										basename(df_media_courant)
									),
									mustWork = FALSE,
									winslash = "\\"
								) #nécessaire car le dossier media a été remonté dans dossier_sortie
						...
						colonne_mutivaluee = subset(
							df_colonnes,
							name == col,
							select = selectMultiple
						)$selectMultiple
						if ((!is.na(colonne_mutivaluee)) & (colonne_mutivaluee == TRUE)) {
							#Traitement particulier pour les colonnes multivaluee => selectMultiple = TRUE
							if (mode_sortie == const_mode_json) {
								#On transforme la colonne multivaluée au format tableau json
								for (i in 1:nrow(df_data)) {
									df_data[i, position_colonne] = tableau_json(
										df_data[i, position_colonne],
										" "
									)
								}
							} else {
								...
								nb_max_valeur = max(str_count(df_temporaire$multivalue, " ")) +
									1 #on compte le nombre d'occurences du séparateur (espace)
								#Eclater la colonne multivaluée en plusieurs : uuid;multivaluee;var1;var2 ...
								setDT(df_temporaire)[,
									paste("var", 1:nb_max_valeur) := tstrsplit(
										df_temporaire$multivalue,
										" "
									)
								]
								...
								df_data = subset(
									merge(
										x = df_data,
										y = df_temporaire_transpose,
										by = "uuid",
										all.x = TRUE
									),
									select = nom_colonne_origine
								)
							}
							#ecrase le fichier avec le dataframe modifié
							write.table(
		...
		warning = function(e) {
			print(paste(
				"Warning function valide_colonne. Fichier : ",
				df_fichier$nom_fichier,
				" - colonne :",
				col
			))
		...
		error = function(e) {
			print(paste(
				"Erreur function valide_colonne. Fichier : ",
				df_fichier$nom_fichier,
				" - colonne :",
				col
			))
	...
	tryCatch(
		{
			...
			chemin_media = normalizePath(
				file.path(dossier_sortie, const_sous_dossier_tmp, const_dossier_media),
				mustWork = FALSE,
				winslash = "\\"
			)

			if (dir.exists(chemin_media)) {
				#remonte le dossier media depuis le sous-dossier tmp
				if (
					!deplacer_dossier(
						chemin_media,
						normalizePath(
							file.path(dossier_sortie, const_dossier_media),
							mustWork = FALSE,
							winslash = "\\"
						)
					)
			...
			doublons = which(duplicated(subset(
				schema_formulaire_courant,
				select = c("nom_fichier", "niveau", "fichier_parent")
			))) #Liste des doublons (sur 3 colonnes)
			listing_fichier = subset(
				schema_formulaire_courant,
				select = c("nom_fichier", "niveau", "fichier_parent")
			)[-doublons, ] #Sans les doublons

			for (i in 1:nrow(listing_fichier)) {
				df_fichier_courant = listing_fichier[i, ]
				df_colonnes_formulaire_courant = subset(
					schema_formulaire_courant,
					nom_fichier == df_fichier_courant$nom_fichier
				)

				normalise_fichier(
					df_fichier = df_fichier_courant,
			...
			unlink(
				normalizePath(file.path(dossier_sortie, const_sous_dossier_tmp)),
				recursive = TRUE
			)
			return(TRUE)
		},
	...
	tryCatch(
		{
			...
			listing_fichier = listing_fichier[
				order(
					listing_fichier$niveau,
					listing_fichier$fichier_parent,
					decreasing = TRUE
				),
			]
			...
			for (i in 1:nrow(listing_fichier)) {
				if (as.numeric(listing_fichier[i, ]$niveau) > 0) {
					...
					fichier_enfant = normalizePath(
						file.path(dossier_sortie, listing_fichier[i, ]$nom_fichier),
						mustWork = TRUE,
						winslash = "\\"
					)
					fichier_parent = normalizePath(
						file.path(dossier_sortie, listing_fichier[i, ]$fichier_parent),
						mustWork = TRUE,
						winslash = "\\"
					)
			...
			liste_colonnes_formulaire_courant = subset(
				schema_formulaire_courant,
				(type != const_type_repeat) & (name != "")
			)$name
			liste_colonnes_sortie = c(
				df_obligatoire_api$colonne_sortie,
				liste_colonnes_formulaire_courant
			)

			fichier_final = normalizePath(
				file.path(dossier_sortie, listing_fichier[i, ]$nom_fichier),
				mustWork = TRUE,
				winslash = "\\"
			)
		...
		warning = function(e) {
			print(paste(
				"Warning function fusionne_csv. Fichier : ",
				listing_fichier[i, ]$nom_fichier
			))
			print(e)
			return(TRUE)
			#return(FALSE)
		...
		error = function(e) {
			print(paste(
				"Erreur function fusionne_csv. Fichier : ",
				listing_fichier[i, ]$nom_fichier
			))
			print(e)
			return(FALSE)
...
recupere_soumission <- function(
	...
	if ((mode_full == TRUE) & (mode_sortie != const_mode_multicsv)) {
		#Le mode full n'est compatible qu'avec le mode de sortie multicsv
		print("Le mode full ne peut être associé qu'au mode de sortie multicsv")
		return(FALSE)
	}
	#Existence dossier vide
	if (dir.exists(dossier_sortie)) {
		if (length(list.files(dossier_sortie)) > 0) {
			#Dossier non vide
	...
	if (!initialise_service_recuperation(nom_du_projet, nom_du_formulaire)) {
		print(paste0(
			"problème initialisation extraction : ",
			nom_du_projet,
			" - ",
			nom_du_formulaire
		))
	...
	if (formulaire_simple) {
		if ((mode_sortie != const_mode_csv) && (mode_sortie != const_mode_json)) {
			print(paste(
				...
				"et",
				const_mode_json,
				"sont supportés pour un formulaire simple :",
				mode_sortie
			))
			return(FALSE)
		}
	...
	if (mode_extraction == const_mode_zip) {
		#Récupération archive zip et décompression
		if (filtre_date != "") {
		...
		unzip(
			...
			files = NULL,
			exdir = normalizePath(
				file.path(dossier_sortie, const_sous_dossier_tmp),
				mustWork = FALSE
			)
		)
		unlink(nom_archive)
		...
		for (soumission in listing_tables$name) {
			data = odata_submission_get(
				...
				local_dir = normalizePath(
					file.path(dossier_sortie, const_sous_dossier_tmp, "media"),
					mustWork = FALSE
				)
			)
			...
			for (colonne in colnames(data)) {
				for (champ_geom in liste_champ_geom$ruodk_name) {
					if (colonne == champ_geom) {
						if (class(data[[colonne]]) == "list") {
							#Le champ geopoint a été rensiegné au moins une fois
							...
							if (exists("infos_champ_geo")) {
								...
								infos_champ_geo = rbind(
									infos_champ_geo,
									data.frame(nom_champ_geom = colonne, renseigne = TRUE)
								)
							} else {
								#On créé le listing
								infos_champ_geo = data.frame(
									nom_champ_geom = colonne,
									renseigne = TRUE
								)
							}
						} else {
							#La colonne n'est pas de type list ce qui signifie qu'elle n'a jamais été renseignée et que la table récupérée
							...
							if (exists("infos_champ_geo")) {
								...
								infos_champ_geo = rbind(
									infos_champ_geo,
									data.frame(nom_champ_geom = colonne, renseigne = FALSE)
								)
							} else {
								#On créé le listing
								infos_champ_geo = data.frame(
	...
	if (exists("infos_champ_geo")) {
		schema_du_formulaire = schema_formulaire(
			mode_extraction = mode_extraction,
			info_geom = infos_champ_geo
		)
	} else {
	...
	if (
		finalise(
			...
			dossier_sortie = dossier_sortie,
			mode_full = mode_full,
			schema_formulaire_courant = schema_du_formulaire
		) ==
			FALSE
	) {
	...
	if (mode_sortie == const_mode_json) {
		if (mode_extraction == const_mode_zip) {
			#En mode zip il reste un fichier csv qui porte le nom du formulaire
			fichier_unique = normalizePath(
				file.path(
					dossier_sortie,
					paste0(fid_formulaire(nom_du_projet, nom_du_formulaire), ".csv")
				),
				mustWork = TRUE,
				winslash = "\\"
			)
		} else {
			#Mode API l'unique fichier porte un nom fixe
			fichier_unique = normalizePath(
				file.path(dossier_sortie, const_nom_fichier_submissions),
				mustWork = TRUE,
				winslash = "\\"
			)
		}
		if (formulaire_simple) {
			#Simple conversion de l'unique fichier de sortie en JSON
			transform_file_json(fichier_unique, pretty = TRUE, purge = TRUE)
			print(paste(
				"Traitement achevé - les résultats sont disponibles dans le dosssier",
				dossier_sortie
			))
			return(TRUE)
		} else {
			fusionne_json(
				...
				mode_sortie = mode_sortie,
				dossier_sortie = dossier_sortie,
				fichier_principal = basename(fichier_unique),
				schema_formulaire_courant = schema_du_formulaire
			)
			print(paste(
				"Traitement achevé - les résultats sont disponibles dans le dosssier",
	...
	if (mode_sortie == const_mode_sql) {
		print(
			"Je dois encore générer une base SQL lite. Traitement non implémentée"
		)
		return(FALSE)
	}
	...
	if (mode_sortie == const_mode_csv) {
		#Au vu du test précédent on est forcement en présence d'un formulaire avec repetitions => fusion
		fusionne_csv(
			mode_extraction = mode_extraction,
			mode_sortie = mode_sortie,
			dossier_sortie = dossier_sortie,
